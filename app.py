...
class User(db.Model):
    ...
    def to_invited_dict(self):
        return {
            "id": self.id,
            "username": self.username,
            "inviteDate": self.created_at.isoformat(),
            "isActive": bool(self.activated_at and self.coins > 0)
        }
...
def ensure_user(identifier):
    ...
    we create a new user with username "user_<identifier>".
    If identifier looks non-numeric, treat as username and create if missing.
